...
const requireCompanyAccess = (req, res, next) => {
  ...
  if (requestedCompanyId && requestedCompanyId !== req.user.companyId) {
    return res.status(403).json({ message: 'Access denied. Cannot access other company data.' });
  }
...
app.get('/api/invoices', authenticateToken, async (req, res) => {
  try {
    ...
    const invoices = result.recordset.map(invoice => {
      ...
      const items = invoice.Items ? JSON.parse(invoice.Items).map(item => ({
        itemID: item.ItemID,
    ...
    console.log('Returning invoices with IDs:', invoices.map(inv => ({ id: inv.invoiceID, type: typeof inv.invoiceID })));
...
app.post('/api/invoices', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('User info:', { role: req.user.role, companyId: req.user.companyId });
    ...
    try {
      // Calculate totals
      const totalAmount = items.reduce((sum, item) => sum + parseFloat(item.totalValues || 0), 0);
      const totalSalesTax = items.reduce((sum, item) => sum + parseFloat(item.salesTaxApplicable || 0), 0);
      const totalFurtherTax = items.reduce((sum, item) => sum + parseFloat(item.furtherTax || 0), 0);
      ...
      for (const item of items) {
        await transaction.request()
          ...
          .input('valueSalesExcludingST', sql.Decimal(18, 2), item.valueSalesExcludingST)
          .input('fixedNotifiedValueOrRetailPrice', sql.Decimal(18, 2), item.fixedNotifiedValueOrRetailPrice || 0)
          .input('salesTaxApplicable', sql.Decimal(18, 2), item.salesTaxApplicable || 0)
          .input('salesTaxWithheldAtSource', sql.Decimal(18, 2), item.salesTaxWithheldAtSource || 0)
      ...
      const createdInvoice = await pool.request()
        .input('invoiceId', sql.UniqueIdentifier, invoiceId)
        .query(`
          SELECT 
            i.*,
      ...
      const invoice = {
        ...
        items: createdInvoice.recordset[0].Items ? JSON.parse(createdInvoice.recordset[0].Items) : []
      };
  ...
  try {
    ...
    try {
      ...
      const invoice = {
        ...
        items: updatedInvoice.recordset[0].Items ? JSON.parse(updatedInvoice.recordset[0].Items) : []
      };
...
app.delete('/api/invoices/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    console.log('Delete request received for invoice ID:', id, 'Type:', typeof id, 'Length:', id.length);
    const pool = await sql.connect(dbConfig);
    const transaction = new sql.Transaction(pool);
    ...
    try {
      ...
      const result = await transaction.request()
        .input('invoiceId', sql.UniqueIdentifier, id)
        .input('companyId', sql.UniqueIdentifier, req.user.companyId)
        .query('DELETE FROM Invoices WHERE InvoiceID = @invoiceId AND CompanyID = @companyId');
...
app.post('/api/scenarios/lookup', authenticateToken, async (req, res) => {
  try {
    ...
    if (!businessActivities || !sectors || !Array.isArray(businessActivities) || !Array.isArray(sectors)) {
    ...
    const businessActivityParams = businessActivities.map((_, index) => `@businessActivity${index}`).join(', ');
    const sectorParams = sectors.map((_, index) => `@sector${index}`).join(', ');
    ...
    result.recordset.forEach(row => {
      if (row.ApplicableScenarios) {
        const scenarios = row.ApplicableScenarios.split(',').map(s => s.trim());
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    console.log('Database query result:', { found: result.recordset.length > 0, username });
...
app.post('/api/users', authenticateToken, async (req, res) => {
  try {
    const { username, password, email, firstName, lastName, role, companyId } = req.body;
    ...
    if (req.user.role === 'SUPER_ADMIN') {
      ...
      if (companyId !== req.user.companyId) {
        return res.status(403).json({ message: 'Access denied. Can only create users for your company.' });
      }
    ...
    // Validate role
    const validRoles = ['SUPER_ADMIN', 'COMPANY_ADMIN', 'ADMIN', 'ACCOUNTANT', 'SALES_PERSON'];
    ...
    const result = await request
      ...
      .input('companyId', sql.UniqueIdentifier, companyId || null)
      .query(`
...
app.put('/api/users/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const existingUser = await checkRequest
      ...
      .query('SELECT UserID, Role, CompanyID FROM Users WHERE UserID = @userId');
    ...
    if (req.user.role === 'SUPER_ADMIN') {
      ...
      if (userToUpdate.CompanyID !== req.user.companyId) {
        return res.status(403).json({ message: 'Access denied. Can only update users from your company.' });
      }
      // Cannot update admin users
      if (userToUpdate.Role === 'SUPER_ADMIN' || userToUpdate.Role === 'COMPANY_ADMIN') {
        return res.status(403).json({ message: 'Access denied. Cannot update admin users.' });
      }
    } else {
      return res.status(403).json({ message: 'Access denied. Admin role required.' });
    }
...
app.delete('/api/users/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const existingUser = await checkRequest
      .input('userId', sql.UniqueIdentifier, id)
      .query('SELECT UserID, Role, CompanyID FROM Users WHERE UserID = @userId AND IsActive = 1');
    ...
    if (req.user.role === 'SUPER_ADMIN') {
      ...
      if (userToDelete.CompanyID !== req.user.companyId) {
        return res.status(403).json({ message: 'Access denied. Can only delete users from your company.' });
      }
      // Cannot delete admin users
      if (userToDelete.Role === 'SUPER_ADMIN' || userToDelete.Role === 'COMPANY_ADMIN') {
...
app.put('/api/users/:id/password', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.id !== id && req.user.role !== 'SUPER_ADMIN' && req.user.role !== 'COMPANY_ADMIN') {
      return res.status(403).json({ message: 'Access denied. Can only change your own password.' });
    }
    ...
    const existingUser = await checkRequest
      .input('userId', sql.UniqueIdentifier, id)
      .query('SELECT UserID, PasswordHash, CompanyID FROM Users WHERE UserID = @userId AND IsActive = 1');
    ...
    if (req.user.id === id && currentPassword) {
      const isValidPassword = await bcrypt.compare(currentPassword, userToUpdate.PasswordHash);
      if (!isValidPassword) {
    ...
    if (req.user.role === 'COMPANY_ADMIN' && req.user.id !== id) {
      if (userToUpdate.CompanyID !== req.user.companyId) {
        return res.status(403).json({ message: 'Access denied. Can only change passwords for users in your company.' });
      }
    }
...
app.post('/api/auth/switch-company', authenticateToken, requireRole(['SUPER_ADMIN']), async (req, res) => {
  try {
    ...
    const company = await request
      .input('companyId', sql.UniqueIdentifier, companyId)
      .query('SELECT CompanyID, Name FROM Companies WHERE CompanyID = @companyId AND IsActive = 1');
...
app.get('/api/companies', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const companies = result.recordset.map(company => ({
      ...
      businessActivity: company.BusinessActivity ? JSON.parse(company.BusinessActivity) : [],
...
app.get('/api/companies/fbr-tokens', authenticateToken, requireRole(['SUPER_ADMIN']), async (req, res) => {
  try {
...
app.post('/api/companies', authenticateToken, async (req, res) => {
  ...
  const { name, ntnNumber, cnic, address, city, province, contactPerson, contactEmail, contactPhone, businessActivity, sector } = req.body;
  
  // Validate required fields
  if (!name || !ntnNumber || !address || !city || !province || !contactPerson || !contactEmail || !contactPhone) {
    return res.status(400).json({ message: 'All fields are required' });
  }
  ...
  try {
    ...
    const result = await request
      ...
      .input('businessActivity', sql.NVarChar(sql.MAX), businessActivity ? JSON.stringify(businessActivity) : null)
      .input('sector', sql.NVarChar(sql.MAX), sector ? JSON.stringify(sector) : null)
      .query(`
    ...
    const formattedCompany = {
      ...
      businessActivity: newCompany.BusinessActivity ? JSON.parse(newCompany.BusinessActivity) : [],
...
app.put('/api/companies/:id', authenticateToken, async (req, res) => {
  ...
  const { id } = req.params;
  const { name, ntnNumber, cnic, address, city, province, contactPerson, contactEmail, contactPhone, businessActivity, sector, isActive } = req.body;
  ...
  try {
    ...
    const formattedCompany = {
      ...
      businessActivity: updatedCompany.BusinessActivity ? JSON.parse(updatedCompany.BusinessActivity) : [],
...
app.get('/api/companies/:id/fbr-token', authenticateToken, requireCompanyAccess, async (req, res) => {
  const { id } = req.params;
...
app.put('/api/companies/:id/fbr-token', authenticateToken, requireCompanyAccess, async (req, res) => {
  const { id } = req.params;
  const { fbrToken } = req.body;
  
  // Validate token
  if (!fbrToken || typeof fbrToken !== 'string' || fbrToken.trim().length === 0) {
    return res.status(400).json({ 
      success: false,
...
app.delete('/api/companies/:id/fbr-token', authenticateToken, requireCompanyAccess, async (req, res) => {
  const { id } = req.params;
...
app.get('/api/inventory', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role === 'SUPER_ADMIN' && req.headers['x-company-id']) {
      companyId = req.headers['x-company-id'];
      console.log('Super admin requesting inventory for company ID:', companyId);
    }
    console.log('Fetching inventory for company ID:', companyId, 'User role:', req.user.role);
    ...
    const customers = result.recordset.map(customer => ({
      ...customer,
      businessActivity: customer.BusinessActivity ? JSON.parse(customer.BusinessActivity) : [],
...
app.post('/api/inventory', authenticateToken, async (req, res) => {
  const { productCode, productName, category, currentStock, minStock, unitPrice } = req.body;
  
  // Validate required fields
  if (!productCode || !productName || !category || currentStock === undefined || minStock === undefined || unitPrice === undefined) {
    return res.status(400).json({
      success: false,
...
app.get('/api/items', authenticateToken, async (req, res) => {
  try {
    ...
    let companyId = req.user.companyId;
    if (req.user.role === 'SUPER_ADMIN' && req.headers['x-company-id']) {
      console.log('Super admin requesting items for company ID:', req.headers['x-company-id']);
      companyId = req.headers['x-company-id'];
    }
    
    console.log('Final companyId to use:', companyId, 'Type:', typeof companyId);
...
app.post('/api/items', authenticateToken, async (req, res) => {
  try {
    const { hsCode, description, unitPrice, purchaseTaxValue, salesTaxValue, uom } = req.body;
...
app.get('/api/purchases', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role === 'SUPER_ADMIN' && req.headers['x-company-id']) {
      companyId = req.headers['x-company-id'];
      console.log('Super admin requesting purchases for company ID:', companyId);
    }
    console.log('Fetching purchases for company ID:', companyId, 'User role:', req.user.role);
...
app.post('/api/purchases', authenticateToken, async (req, res) => {
  const { poNumber, poDate, crNumber, date, vendorId, vendorName, items, totalAmount, status } = req.body;
...
app.get('/api/purchases/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const purchase = {
      ...result.recordset[0],
      items: result.recordset[0].Items ? JSON.parse(result.recordset[0].Items) : []
    };
...
app.get('/api/customers', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role === 'SUPER_ADMIN' && req.headers['x-company-id']) {
      companyId = req.headers['x-company-id'];
      console.log('Super admin requesting customers for company ID:', companyId);
    }
    console.log('Fetching customers for company ID:', companyId, 'User role:', req.user.role);
    ...
    const formattedVendors = result.recordset.map(vendor => ({
      ...vendor,
      businessActivity: vendor.BusinessActivity ? JSON.parse(vendor.BusinessActivity) : [],
...
app.post('/api/customers', authenticateToken, async (req, res) => {
  try {
    const { buyerNTNCNIC, buyerBusinessName, buyerProvince, buyerAddress, buyerRegistrationType, buyerRegistrationNo, buyerEmail, buyerCellphone, contactPersonName, businessActivity, sector } = req.body;
    
    if (!buyerNTNCNIC || !buyerBusinessName || !buyerProvince || !buyerAddress || !buyerRegistrationType) {
      return res.status(400).json({
        success: false,
    ...
    const result = await pool.request()
      ...
      .input('buyerRegistrationNo', sql.NVarChar(50), buyerRegistrationNo || null)
    ...
    const formattedCustomer = {
      ...newCustomer,
      businessActivity: newCustomer.BusinessActivity ? JSON.parse(newCustomer.BusinessActivity) : [],
...
app.put('/api/customers/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const formattedCustomer = {
      ...updatedCustomer,
      businessActivity: updatedCustomer.BusinessActivity ? JSON.parse(updatedCustomer.BusinessActivity) : [],
...
app.post('/api/vendors', authenticateToken, async (req, res) => {
  try {
    const { vendorName, vendorNTN, vendorCNIC, contactPersonName, vendorAddress, vendorPhone, vendorEmail, businessActivity, sector } = req.body;
    
    if (!vendorName || !vendorNTN || !vendorAddress || !vendorPhone || !vendorEmail) {
      return res.status(400).json({
        success: false,
...
app.patch('/api/vendors/:id/toggle-status', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
...
app.get('/api/reports/sales', authenticateToken, async (req, res) => {
  try {
    const { reportType = 'sales', dateRange = 'monthly', startDate, endDate } = req.query;
    ...
    if (isDbConnected) {
      ...
      const mockSalesData = [
        { period: '2024-01', sales: 125000, purchases: 87500, profit: 12500, orders: 45 },
        { period: '2024-02', sales: 135000, purchases: 94500, profit: 13500, orders: 52 },
        { period: '2024-03', sales: 142000, purchases: 99400, profit: 14200, orders: 48 },
        { period: '2024-04', sales: 158000, purchases: 110600, profit: 15800, orders: 61 },
        { period: '2024-05', sales: 167000, purchases: 116900, profit: 16700, orders: 55 },
        { period: '2024-06', sales: 175000, purchases: 122500, profit: 17500, orders: 63 }
      ];
      ...
      const mockTopCustomers = [
        { name: 'ABC Corporation', amount: 85000, orders: 12, percentage: 28.3 },
        { name: 'XYZ Industries', amount: 67000, orders: 9, percentage: 22.3 },
        { name: 'Tech Solutions Ltd', amount: 52000, orders: 8, percentage: 17.3 },
        { name: 'Global Traders', amount: 43000, orders: 6, percentage: 14.3 },
        { name: 'Prime Enterprises', amount: 38000, orders: 7, percentage: 12.7 }
      ];
...
app.get('/api/dashboard/stats', authenticateToken, async (req, res) => {
  try {
    ...
    if (isDbConnected) {
      ...
      const salesChartResult = await pool.request()
        .input('companyId', sql.UniqueIdentifier, companyId)
      ...
      const activities = activitiesResult.recordset.map(activity => ({
        ...
        description: `Invoice ${activity.InvoiceNumber} - ${activity.CustomerName || 'Unknown Customer'}`,
...
app.get('/api/reports/fbr-compliance-summary', authenticateToken, async (req, res) => {
  try {
    ...
    const businessActivities = company.BusinessActivity ? JSON.parse(company.BusinessActivity) : [];
    ...
    for (const activity of businessActivities) {
      for (const sector of sectors) {
        ...
        if (scenarioResult.recordset.length > 0) {
          const scenarios = JSON.parse(scenarioResult.recordset[0].applicable_scenarios);
    ...
    const complianceRate = salesData.TotalInvoices > 0 ? 
      (salesData.FBRSubmittedInvoices / salesData.TotalInvoices * 100).toFixed(2) : 0;
    ...
    res.status(500).json({ message: 'Server error while generating compliance report' });
  }
});

// Get detailed FBR scenario usage report
app.get('/api/reports/fbr-scenario-usage', authenticateToken, async (req, res) => {
  try {
    const companyId = req.user.role === 'SUPER_ADMIN' ? req.query.companyId : req.user.companyId;
    ...
    res.json({
      ...
      summary: {
        ...
        fbrSubmitted: salesResult.recordset.filter(s => s.FBRInvoiceNumber).length,
        totalAmount: salesResult.recordset.reduce((sum, s) => sum + (s.TotalAmount || 0), 0)
      }
    });

  } catch (error) {
    console.error('Error generating FBR scenario usage report:', error);
    res.status(500).json({ message: 'Server error while generating scenario usage report' });
  }
});

// Get FBR compliance trends report
app.get('/api/reports/fbr-compliance-trends', authenticateToken, async (req, res) => {
  try {
    const companyId = req.user.role === 'SUPER_ADMIN' ? req.query.companyId : req.user.companyId;
    ...
    const trends = trendsResult.recordset.map(trend => ({
      ...
      complianceRate: trend.TotalInvoices > 0 ? 
        (trend.FBRSubmittedInvoices / trend.TotalInvoices * 100).toFixed(2) : 0,
    ...
    res.status(500).json({ message: 'Server error while generating compliance trends' });
  }
});
...
app.post('/api/migrate/vendor-contactpersonname', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'ContactPersonName column and index added to Vendors table successfully'
    });
...
connectToDatabase().then((connected) => {
  ...
  app.listen(PORT, () => {
    if (connected) {
      ...
      console.log(`Server running on port ${PORT} with mock data (no database connection)`);
      console.log('Note: All data is mocked for demo purposes. No actual database operations will be performed.');
    }
  });
}).catch(err => {
