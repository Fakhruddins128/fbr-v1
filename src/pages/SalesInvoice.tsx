 ...
 // SN026: Sale of Standard Rate Goods to Retail Consumer Buyers
  ...
  const saveInvoice = async () => {
    ...
    try {
      ...
      // Calculate Extra Tax and Further Tax based on invoice-level percentages
      // Extra Tax = Total Val. Excl. S.Tax × Extra Tax % ÷ 100
      const totalValueExclST = formData.items.reduce((sum, item) => sum + item.valueSalesExclST, 0);
      const extraTaxPercent = formData.extraTaxPercent || 0;
      const extraTaxAmount = (totalValueExclST * extraTaxPercent) / 100;

      // Further Tax = Total Val. Incl. S.Tax × Further Tax % ÷ 100
      const totalValueInclST = formData.items.reduce((sum, item) => sum + (item.valueSalesExclST + item.salesTax), 0);
      const furtherTaxPercent = formData.furtherTaxPercent || 0;
      const furtherTaxAmount = (totalValueInclST * furtherTaxPercent) / 100;

      // Final Invoice Total = Total Val. Incl. S.Tax + Extra Tax Amount + Further Tax Amount
      const finalInvoiceTotal = totalValueInclST + extraTaxAmount + furtherTaxAmount;
      ...
      buyerNTN: formData.buyerNTN,
        ...
        totalFurtherTax,
        totalDiscount,
        // New tax fields at invoice level
        extraTaxPercent,
        extraTaxAmount,
        furtherTaxPercent,
        furtherTaxAmount,
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Grid container spacing={3}>
          ...
          {/* Extra Tax % and Further Tax % Fields */}
          <Grid size={{ xs: 12, md: 6 }}>
            <TextField
              fullWidth
              label="Extra Tax %"
              type="number"
              value={formData.extraTaxPercent}
              onChange={(e) => handleInputChange('extraTaxPercent', parseFloat(e.target.value) || 0)}
              size="small"
              inputProps={{ min: 0, step: 0.01 }}
              helperText="Applied on Total Val. Excl. S.Tax"
              disabled={isInvoiceSentToFBR()}
            />
          </Grid>
          <Grid size={{ xs: 12, md: 6 }}>
            <TextField
              ...
              label="Further Tax %"
              type="number"
              value={formData.furtherTaxPercent}
              onChange={(e) => handleInputChange('furtherTaxPercent', parseFloat(e.target.value) || 0)}
              size="small"
              inputProps={{ min: 0, step: 0.01 }}
              helperText="Applied on Total Val. Incl. S.Tax"
              disabled={isInvoiceSentToFBR()}
            />
          </Grid>
      ...
      {/* Invoice Summary */}
      <Paper sx={{ mt: 3, p: 3, bgcolor: '#f8f9fa', border: '1px solid #e9ecef' }}>
        ...
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 6 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
              <Typography variant="body2" color="text.secondary">Total Val. Excl. S.Tax:</Typography>
              <Typography variant="body2" fontWeight="medium">
                {formData.items.reduce((sum, item) => sum + item.valueSalesExclST, 0).toFixed(2)}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
              <Typography variant="body2" color="text.secondary">Total S.Tax:</Typography>
              <Typography variant="body2" fontWeight="medium">
                {formData.items.reduce((sum, item) => sum + item.salesTax, 0).toFixed(2)}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
              <Typography variant="body2" color="text.secondary">Total Val. Incl. S.Tax:</Typography>
              <Typography variant="body2" fontWeight="medium">
                {formData.items.reduce((sum, item) => sum + (item.valueSalesExclST + item.salesTax), 0).toFixed(2)}
              </Typography>
            </Box>
          </Grid>
          <Grid size={{ xs: 12, md: 6 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
              <Typography variant="body2" color="text.secondary">
                Extra Tax ({formData.extraTaxPercent}%):
              </Typography>
              <Typography variant="body2" fontWeight="medium">
                {((formData.items.reduce((sum, item) => sum + item.valueSalesExclST, 0) * (formData.extraTaxPercent || 0)) / 100).toFixed(2)}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
              <Typography variant="body2" color="text.secondary">
                Further Tax ({formData.furtherTaxPercent}%):
              </Typography>
              <Typography variant="body2" fontWeight="medium">
                {((formData.items.reduce((sum, item) => sum + (item.valueSalesExclST + item.salesTax), 0) * (formData.furtherTaxPercent || 0)) / 100).toFixed(2)}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5, borderTop: '1px solid #e0e0e0', pt: 1, mt: 0.5 }}>
              ...
              <Typography variant="body1" fontWeight="bold" color="primary">
                {(() => {
                  const totalValExclST = formData.items.reduce((sum, item) => sum + item.valueSalesExclST, 0);
                  const totalSalesTax = formData.items.reduce((sum, item) => sum + item.salesTax, 0);
                  const totalValInclST = totalValExclST + totalSalesTax;
                  const extraTax = (totalValExclST * (formData.extraTaxPercent || 0)) / 100;
                  const furtherTax = (totalValInclST * (formData.furtherTaxPercent || 0)) / 100;
                  return (totalValInclST + extraTax + furtherTax).toFixed(2);
                })()}
              </Typography>
            </Box>
          </Grid>
